...
export const Index: Record<string, any> = {`
  for (const item of registry.items) {
    const resolveFiles = item.files?.map(
      (file) => `registry/aliimam/${file.path}`
    )
    if (!resolveFiles) {
      continue
    }
    ...
    const componentPath = item.files?.[0]?.path
      ? `@/registry/aliimam/${item.files[0].path}`
      : ""
  ...
  "${item.name}": {
    ...
    files: [${item.files?.map((file) => {
      const filePath = `registry/aliimam/${typeof file === "string" ? file : file.path}`
...
async function buildRegistryJsonFile() {
  // 1. Add registry/aliimam prefix for the build to work
  const fixedRegistry = {
    ...registry,
    items: registry.items.map((item) => {
      const files = item.files?.map((file) => {
        return {
          ...file,
          path: `registry/aliimam/${file.path}`,
        }
      })
  ...
  // 3. Copy the registry.json to the www/public/r/styles/aliimam directory.
  await fs.cp(
    path.join(process.cwd(), "registry.json"),
    path.join(
      process.cwd(),
      "../www/public/r/styles/aliimam/registry.json"
    ),
    { recursive: true }
  )
}
  ...
  return new Promise((resolve, reject) => {
    exec(
      ...
      async (error, stdout, stderr) => {
        ...
        // Post-process the generated JSON files to remove registry/aliimam prefix
    ...
    if (json.files) {
      ...
      json.files.forEach((file: any) => {
        if (typeof file === "object" && file.path && file.target) {
          ...
          const sourcePath = file.path.replace(/^registry\/aliimam\//, "")
      ...
      json.files = json.files.map((file: any) => {
        if (typeof file === "object" && file.path) {
          // Remove registry/aliimam/ prefix
          let cleanPath = file.path.replace(/^registry\/aliimam\//, "")
...
function rewriteImports(
  ...
  // Rewrite imports from @/registry/aliimam/ui/... to @/components/ui/...
  content = content.replace(
    /@\/registry\/aliimam\/ui\//g,
    "@/components/ui/"
  )

  // Rewrite imports from @/registry/aliimam/examples/... to @/components/examples/...
  content = content.replace(
    /@\/registry\/aliimam\/examples\//g,
    "@/components/examples/"
  )

  // Rewrite imports from @/registry/aliimam/lib/... to @/lib/...
  content = content.replace(/@\/registry\/aliimam\/lib\//g, "@/lib/")
...
async function buildAllJson() {
  ...
  for (const component of uiComponents) {
    ...
    if (component.files) {
      for (const file of component.files) {
        const filePath = `registry/aliimam/${file.path}`
        ...
        // Clean path - remove registry/aliimam/ prefix
  ...
  const allJson = {
    ...
    name: "all",
    type: "registry:ui",
    description: "All UI components from Ali Imam UI",
