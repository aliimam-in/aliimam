...
type LogoFilterContext = {
  ...
  setVariantTab: (v: "outline" | "filled" | "circle") => void
  counts: {
    filtered: number      // Search result count
    outline: number       // Outline tab count
    filled: number        // Filled tab count
    circle: number        // Circle tab count
  }
}
...
export function IconFilterProvider({ children }: { children: React.ReactNode }) {
  ...
  const [variantTab, setVariantTab] = useState<"outline" | "filled" | "circle">("outline")
  ...
  const counts = useMemo(() => {
    ...
    Object.entries(allLogos).forEach(([cat, logos]) => {
      Object.values(logos as any).forEach((entry: any) => {
        ...
        else if (v === "circle") variantCounts.circle++
