...
for (const icons of Object.values(allIcons) as Record<string, any>[]) {
...
export function IconGrid({ selectedLogo, onSelectLogo }: LogoGridProps) {
  ...
  const allGridEntries = useMemo(() => {
    ...
    ;(Object.entries(allIcons) as [string, Record<string, any>][]).forEach(
      ([cat, logos]) => {
          ...
          return (
            <Tooltip delayDuration={300} key={baseId}>
              <TooltipTrigger asChild>
                ...
                >
                  {/* exact iconId pass karo — no variant guessing */}
                  <Icons strokeWidth={1} name={iconId} size={30} />
                </button>
              </TooltipTrigger>
