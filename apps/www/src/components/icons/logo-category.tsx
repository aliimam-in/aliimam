...
function matchesVariantTab(variant: string, tab: "icon" | "wordmark" | "flags" | "cards" | "stickers"): boolean {
...
export function LogoCategoryTabs() {
  const { setCategory, variantTab } = useLogoFilter()
  ...
  const handleClick = (cat: string | null) => {
    ...
    window.history.pushState(null, "", `/logos${search}${hash}`)
