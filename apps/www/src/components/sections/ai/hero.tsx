import React from "react"
import Link from "next/link"
import {
  PageHeader,
  PageHeaderDescription,
  PageHeaderHeading,
} from "@/src/components/layout/page-header"
import { ImageZoom } from "@/src/components/ui/image-zoom"
import { Plus } from "lucide-react"

import { Shadcn, TwentyFirst } from "../../logos"

export function Hero() {
  return (
    <div className="relative pb-20">
      <PageHeader>
        <PageHeaderHeading>About AI</PageHeaderHeading>
        <PageHeaderDescription>
          I’m a passionate in Design and Code based in Bokaro Steel City, India.
          Also, It’s not that hard to find my contact just search Ali Imam
          designer.
        </PageHeaderDescription>
      </PageHeader>
      <div>
        <h1 className="text-center">Founder</h1>
        <div className="container grid items-center justify-center lg:grid-cols-3">
          <h1 className="mt-6 text-center">
            I&apos;m Ali, Creative Design Engineer.
          </h1>

          <div className="border-primary/10 w-[320px] relative mx-auto my-6 flex h-fit flex-col border p-4">
            <Plus className="text-primary absolute -top-4 -left-4" />
            <Plus className="text-primary absolute -bottom-4 -left-4" />
            <Plus className="text-primary absolute -top-4 -right-4" />
            <Plus className="text-primary absolute -right-4 -bottom-4" />
            <ImageZoom>
              <img
                src="/ai.jpg"
                alt="Your Image"
                height={700}
                width={700}
                className="h-[400px] w-[300px] object-cover"
              />
              <div className="-mt-20 md:-mt-26 xl:-mt-27 2xl:-mt-23 relative z-10 bg-gradient-to-b from-black/0 to-black text-white">
                <h1 className="z-20 items-center text-center text-[40px] font-black tracking-tighter uppercase md:text-[54px]">
                  Ali Imam
                </h1>{" "}
              </div>
            </ImageZoom>
          </div>
          <h1 className="mt-6 text-center">Design Without Limits.</h1>
        </div>
      </div>
      <div className="relative">
        <div className="flex w-full flex-wrap justify-center gap-3 p-3 pt-10">
          <Link
            href={"https://21st.dev/community/aliimam"}
            target="_blank"
            className="hover:bg-muted w-fit cursor-pointer border p-1"
          >
            <div className="flex items-center justify-center pl-3">
              <h1 className="text-xl font-semibold md:text-2xl">
                I&apos;m an Ambassador in
              </h1>
              <TwentyFirst className="size-12 p-2 md:size-14" />
            </div>
          </Link>
          <Link
            href={"https://ui.shadcn.com/docs/directory"}
            target="_blank"
            className="hover:bg-muted w-fit cursor-pointer border p-1"
          >
            <div className="flex items-center justify-center pl-3">
              <h1 className="text-xl font-semibold md:text-2xl">
                Registry Directory in
              </h1>
              <Shadcn className="size-12 p-3 md:size-14" />
            </div>
          </Link>
        </div>
      </div>
    </div>
  )
}
