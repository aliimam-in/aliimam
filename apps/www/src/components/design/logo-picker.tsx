...
async function renderComponentToSVGString(
...
export function LogoPicker({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const mod = (await import("@aliimam/logos")) as unknown as IconExport;
  ...
  const filtered = React.useMemo(() => {
    ...
    return Object.entries(exportsMap).filter(([name]) =>
  ...
  const handleInsert = async (name: string, Comp: React.ComponentType<any>) => {
    try {
      const svg = await renderComponentToSVGString(Comp, 160);
      if (!svg) return;
              ...
              {!loading &&
                filtered.slice(0, 200).map(([name, Comp]) => (
                  ...
                  >
                    ...
                    <span className="line-clamp-1 w-full text-center text-xs">
                      {name}
                    </span>
                  </button>
                ))}
