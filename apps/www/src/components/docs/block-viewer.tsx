...
import {
  ...
  CollapsibleContent,
  CollapsibleTrigger,
} from "@/registry/aliimam/ui/collapsible"
import {
  ResizableHandle,
  ResizablePanel,
  ResizablePanelGroup,
} from "@/registry/aliimam/ui/resizable"
  ...
  return (
    ...
    >
      <div
        id={item.name}
        data-view={view}
        className="group/block-view-wrapper flex min-w-0 scroll-mt-34 flex-col-reverse items-stretch gap-4 overflow-hidden md:flex-col"
        style={
          {
...
function BlockViewerToolbar() {
  ...
  return (
    <div className="hidden w-full items-center gap-2 md:pr-4 lg:flex">
      <Tabs
        value={view}
        onValueChange={(value) => setView(value as "preview" | "code")}
      >
        <TabsList className="grid h-8 grid-cols-2 items-center p-1 *:data-[slot=tabs-trigger]:h-7 *:data-[slot=tabs-trigger]:rounded-sm *:data-[slot=tabs-trigger]:px-2 *:data-[slot=tabs-trigger]:text-xs">
      ...
      <div className="ml-auto flex items-center gap-2">
        <div className="h-8 items-center gap-1.5 border p-1 shadow-none">
          <ToggleGroup
            type="single"
            defaultValue="100"
            onValueChange={(value) => {
    ...
    <div className="hidden group-data-[view=code]/block-view-wrapper:hidden md:h-[(--height)] lg:flex">
      <div className="relative grid w-full gap-4">
        ...
        <ResizablePanelGroup
          ...
          className="after:bg-surface/50 relative z-10 after:absolute after:inset-0 after:right-3 after:z-0"
        >
          <ResizablePanel
            ref={resizablePanelRef}
            className="bg-background relative aspect-[4/2.5] overflow-hidden border md:aspect-auto"
            defaultSize={100}
            minSize={30}
          >
            <BlockViewerIframe />
