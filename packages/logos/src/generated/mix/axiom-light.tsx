/**
 * Auto-generated logo component: Axiom Light (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface AxiomLightLogoProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const AxiomLightLogo = React.forwardRef<SVGSVGElement, AxiomLightLogoProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 358 309"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g clipPath="url(#a)"><path fill="#000" d="M354.75 215.609 278.412 87.8475c-3.501-5.872-12.127-10.6761-19.17-10.6761h-47.659c-11.077 0-15.618-7.5477-10.093-16.7723l26.136-43.6269c2.074-3.4623 2.069-7.72483-.011-11.1835C225.534 2.13003 221.691 0 217.533 0h-66.485c-7.044 0-15.688 4.79349-19.212 10.6524L2.6449 225.448c-3.524953 5.859-3.526307 15.447-.00541 21.307L35.8815 302.08c5.5392 9.217 14.6223 9.228 20.1846.023l25.9742-42.98c5.5636-9.205 14.6452-9.195 20.1847.023l23.548 39.192c3.521 5.86 12.164 10.654 19.207 10.654h153.633c7.04 0 15.685-4.794 19.206-10.654l36.892-61.397c3.521-5.86 3.538-15.459.039-21.332Zm-103.096-6.149c5.505 9.236.945 16.794-10.132 16.794H122.021c-11.077 0-15.609-7.542-10.07-16.76l59.796-99.517c5.539-9.218 14.602-9.217 20.141.001l59.766 99.482Z" /></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h358v309H0z" /></clipPath></defs>
    </svg>
  )
);

AxiomLightLogo.displayName = "AxiomLightLogo";

export const AxiomLightLogoMetadata = {
  id: "axiom-light",
  baseId: "axiom-light",
  variant: "default",
  name: "Axiom Light",
  category: "mix",
  tags: [],
  viewBox: "0 0 358 309",
} as const;

export default AxiomLightLogo;
