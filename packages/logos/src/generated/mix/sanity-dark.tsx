/**
 * Auto-generated logo component: Sanity Dark (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface SanityDarkLogoProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const SanityDarkLogo = React.forwardRef<SVGSVGElement, SanityDarkLogoProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 192 192"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path fill="#fff" d="m160.077 112.697-5.212-9.068-25.206 15.352 27.996-35.644 4.233-2.484-1.047-1.573 1.923-2.457-8.835-7.353-4.043 5.153-81.62 47.752 30.177-36.29 56.208-30.81-5.34-10.322-30.615 16.775 15.075-18.118-8.637-7.61-33.928 40.797-33.698 18.48 25.799-34.106 16.166-8.414-5.134-10.438L47.24 66.836l12.844-16.995-8.972-7.186L24 78.538l.42.336 5.066 10.314 30.057-15.653-27.396 36.21 4.49 3.597 2.67 5.162 31.646-17.35-34.848 41.911 8.637 7.609 1.734-2.086 84.067-49.343-27.911 35.547.456.38-.043.027 5.786 10.066 37.123-22.616-14.295 23.067L141.24 152 164 115.278l-3.923-2.581Z" />
    </svg>
  )
);

SanityDarkLogo.displayName = "SanityDarkLogo";

export const SanityDarkLogoMetadata = {
  id: "sanity-dark",
  baseId: "sanity-dark",
  variant: "default",
  name: "Sanity Dark",
  category: "mix",
  tags: [],
  viewBox: "0 0 192 192",
} as const;

export default SanityDarkLogo;
