/**
 * Auto-generated logo component: Sourcegraph (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface SourcegraphLogoProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const SourcegraphLogo = React.forwardRef<SVGSVGElement, SourcegraphLogoProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="-3 0 262 262"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g>
		<path d="M74.2028387,33.6020645 L131.154581,241.796129 C134.982194,255.787355 149.425548,264.026839 163.408516,260.195097 C177.401806,256.363355 185.633032,241.915871 181.803355,227.92671 L124.84129,19.7264516 C121.011613,5.73935484 106.570323,-2.50219355 92.5873548,1.3316129 C78.6064516,5.15716129 70.3731613,19.6087742 74.2028387,33.6 L74.2028387,33.6020645 Z" fill="#F96316">

</path>
		<path d="M179.662452,32.8134194 L37.0456774,193.858065 C27.4250323,204.717419 28.4304516,221.322323 39.2794839,230.945032 C50.1285161,240.565677 66.7169032,239.562323 76.3375484,228.707097 L218.954323,67.6645161 C228.574968,56.8051613 227.569548,40.2064516 216.720516,30.5816774 C205.867355,20.9527742 189.274839,21.9581935 179.662452,32.8113548 L179.662452,32.8134194 Z" fill="#B200F8">

</path>
		<path d="M18.0645161,122.054194 L221.452387,189.347097 C235.216516,193.899355 250.066581,186.427871 254.618839,172.651355 C259.181419,158.876903 251.707871,144.020645 237.937548,139.462194 L34.555871,72.1754839 C20.7855484,67.6190968 5.94167742,75.0905806 1.38735484,88.8670968 C-3.16283871,102.641548 4.31070968,117.501935 18.0686452,122.054194 L18.0645161,122.054194 Z" fill="#00B4F2">

</path>
	</g>
    </svg>
  )
);

SourcegraphLogo.displayName = "SourcegraphLogo";

export const SourcegraphLogoMetadata = {
  id: "sourcegraph",
  baseId: "sourcegraph",
  variant: "default",
  name: "Sourcegraph",
  category: "mix",
  tags: [],
  viewBox: "-3 0 262 262",
} as const;

export default SourcegraphLogo;
