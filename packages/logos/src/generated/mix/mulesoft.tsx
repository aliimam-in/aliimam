/**
 * Auto-generated logo component: Mulesoft (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface MulesoftLogoProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const MulesoftLogo = React.forwardRef<SVGSVGElement, MulesoftLogoProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 691 209"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g id="Layer_2_1_">
	<g id="Layer_1-2">
		<polygon fill="#00A0DF" points="288.1,115.3 262.3,60.4 248.9,60.4 248.9,148.7 262.3,148.7 262.3,89.7 283.2,132.7 293.1,132.7 
			313.4,89.7 313.4,148.7 326.8,148.7 326.8,60.4 313.4,60.4 		" />
		<path fill="#00A0DF" d="M377.1,124.2c0,9.7-6,14-13,14s-12.6-4.2-12.6-14V85.8h-12.7V126c0,7.1,1.5,13,6.3,17.8
			c3.7,3.6,9,5.7,15.5,5.7c6.3,0.1,12.4-2.5,16.7-7.1v6.3h12.4V85.8h-12.6V124.2z" />
		<path fill="#00A0DF" d="M414.5,130.6V60.4h-12.6v70.9c0,9.3,5.2,17.4,17.2,17.4h7.8v-10.8h-5.6C416.3,137.9,414.5,135.4,414.5,130.6z
			" />
		<path fill="#00A0DF" d="M457.4,85.1c-16.1,0-26.6,11.8-26.6,32.1c0,23.9,12.5,32.2,28.3,32.2c11,0,17.1-3.4,23.5-9.8l-8.1-7.6
			c-4.5,4.5-8.2,6.4-15.2,6.4c-10.3,0-16-6.8-16-17.6H484v-5.6C484,97.5,474.2,85.1,457.4,85.1z M443.2,112.1
			c0.1-3.8,0.4-5.7,1.6-8.6c2.1-5,7.1-8.1,12.5-7.9c5.4-0.2,10.3,3,12.4,7.9c1.2,2.7,1.8,5.6,1.7,8.6H443.2z" />
		<path fill="#00A0DF" d="M546.4,105.4c-3.7-3.3-8.3-5.3-16.5-6.6l-10-1.5c-3.6-0.4-7.1-1.8-9.9-4.1c-2.3-2.1-3.5-5.1-3.3-8.2
			c0-7.8,5.7-13.8,16.1-13.8c7.4,0,13.8,1.6,19.2,6.8l8.6-8.4c-7.6-7.1-15.7-10-27.4-10c-18.4,0-29.5,10.5-29.5,25.9
			c0,7.2,2.1,12.8,6.5,16.9c3.8,3.6,9.5,6.1,16.7,7.1l10.4,1.5c5.2,0.7,7.5,1.6,9.7,3.7s3.5,5.3,3.5,9.2c0,8.6-6.7,13.5-18.2,13.5
			c-9.1,0-16.1-2-22.6-8.4l-8.9,8.8c8.4,8.6,18,11.7,31.2,11.7c18.5,0,31.7-9.7,31.7-25.9C553.6,116,551.3,109.7,546.4,105.4z" />
		<path fill="#00A0DF" d="M587,85.1c-8.5,0-14.6,3.2-18.7,7.4c-6,6.2-7.4,14.1-7.4,24.7s1.5,18.6,7.4,24.8c4.1,4.2,10.2,7.4,18.7,7.4
			s14.8-3.2,18.8-7.4c6-6.2,7.4-14.1,7.4-24.8s-1.5-18.5-7.4-24.7C601.8,88.3,595.5,85.1,587,85.1z M596.3,134.4
			c-2.5,2.4-5.8,3.8-9.3,3.7c-3.4,0.1-6.7-1.3-9.2-3.7c-3.9-3.8-4.3-10.4-4.3-17.2s0.5-13.4,4.3-17.2c2.4-2.4,5.7-3.7,9.2-3.6
			c3.5-0.1,6.8,1.2,9.3,3.6c3.8,3.8,4.3,10.4,4.3,17.2S600.1,130.6,596.3,134.4L596.3,134.4z" />
		<path fill="#00A0DF" d="M626.1,77.3V87h-7.2v9.7h7.2v52.1h12.7V96.6h12.5V87h-12.5v-9c0-4.6,2.2-7.3,6.9-7.3h5.6V59.8h-8
			C631.5,59.8,626.1,68.2,626.1,77.3z" />
		<path fill="#00A0DF" d="M678.9,67.9h-12.6V87H659v9.7h7.2v34.6c0,9.1,5.4,17.5,17.2,17.5h7.6v-10.8h-5.2c-4.7,0-7-2.7-7-7.3v-34H691
			V87h-12.1V67.9z" />
		<path fill="#00A0DF" d="M104.5,0C46.8,0,0,46.8,0,104.5S46.8,209,104.5,209S209,162.2,209,104.5S162.2,0,104.5,0z M104.5,199.8
			c-52.6,0-95.3-42.7-95.3-95.3S51.9,9.2,104.5,9.2s95.3,42.7,95.3,95.3l0,0C199.8,157.1,157.1,199.8,104.5,199.8z" />
		<path fill="#00A0DF" d="M179.8,83.7L179.8,83.7C171.4,52.4,145.3,38,145.3,38l-2.9,4.6l-37.8,59.2l0,0L66.7,42.5L63.8,38
			c0,0-26.1,14.4-34.5,45.8l0,0c-5.8,20.3-3,42.1,7.9,60.2c8,13.7,20,24.6,34.4,31.3l7-26.7c-15.7-7.4-27.1-22.4-27.1-41.7
			c0-9.7,2.8-19.2,8.2-27.3l32.7,51.1h24.3l32.7-51.1c5.4,8.1,8.2,17.6,8.2,27.3c0,19.3-11.4,34.3-27.1,41.7l7,26.7
			c14.4-6.7,26.4-17.6,34.4-31.3C182.7,125.8,185.6,104,179.8,83.7z" />
	</g>
</g>
    </svg>
  )
);

MulesoftLogo.displayName = "MulesoftLogo";

export const MulesoftLogoMetadata = {
  id: "mulesoft",
  baseId: "mulesoft",
  variant: "default",
  name: "Mulesoft",
  category: "mix",
  tags: [],
  viewBox: "0 0 691 209",
} as const;

export default MulesoftLogo;
