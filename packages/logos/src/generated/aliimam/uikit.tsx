/**
 * Auto-generated logo component: Uikit (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface UikitProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const Uikit = React.forwardRef<SVGSVGElement, UikitProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 256 297"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g>
        <path d="M174.656,28.438 L125.717,0 L74.646,31.723 L124.267,59.137 L174.656,28.438 L174.656,28.438 Z M198.059,40.683 L147.52,71.424 L204.8,104.533 L204.8,192 L127.552,235.904 L51.2,192 L51.2,123.733 L0,98.304 L0,221.867 L125.717,296.533 L256,222.422 L256,74.155 L198.059,40.683 L198.059,40.683 Z" fill="#2396F3"></path>
    </g>
    </svg>
  )
);

Uikit.displayName = "Uikit";

export const UikitMetadata = {
  id: "uikit",
  baseId: "uikit",
  variant: "default",
  name: "Uikit",
  category: "aliimam",
  tags: [],
  viewBox: "0 0 256 297",
} as const;

export default Uikit;
