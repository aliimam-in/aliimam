/**
 * Auto-generated logo component: Cakephp Icon (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface CakephpIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const CakephpIcon = React.forwardRef<SVGSVGElement, CakephpIconProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 256 200"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
        <path d="M0,119.882843 C0,141.938825 57.2937323,159.841092 127.979753,159.841092 L127.979753,159.841092 L127.979753,199.783147 C57.2937323,199.783147 0,181.897074 0,159.841092 L0,159.841092 Z M255.999995,119.882843 L255.999995,159.832995 C255.999995,169.249701 245.530687,177.889104 228.09804,184.722895 L228.09804,184.722895 L127.971656,159.832995 L127.971656,119.882843 L228.09804,144.764647 C245.530687,137.938952 256.008088,129.299549 255.999995,119.882843 L255.999995,119.882843 Z M127.979753,0 C198.673871,0 256,17.9184609 256,39.9582488 L256,39.9582488 L256,79.9407884 C256,89.3251066 245.530687,97.9968968 228.106137,104.798301 L228.106137,104.798301 L127.979753,79.9407884 L127.979753,119.882843 C57.2937323,119.882843 0,101.988673 0,79.9407884 L0,79.9407884 L0,39.9582488 C0,17.9184609 57.2937323,0 127.979753,0 Z" fill="#D33C44"></path>
    </g>
    </svg>
  )
);

CakephpIcon.displayName = "CakephpIcon";

export const CakephpIconMetadata = {
  id: "cakephp-icon",
  baseId: "cakephp-icon",
  variant: "default",
  name: "Cakephp Icon",
  category: "aliimam",
  tags: [],
  viewBox: "0 0 256 200",
} as const;

export default CakephpIcon;
