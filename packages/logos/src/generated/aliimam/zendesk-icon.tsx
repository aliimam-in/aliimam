/**
 * Auto-generated logo component: Zendesk Icon (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface ZendeskIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const ZendeskIcon = React.forwardRef<SVGSVGElement, ZendeskIconProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 256 195"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g>
        <path d="M118.249172,51.2326115 L118.249172,194.005605 L0,194.005605 L118.249172,51.2326115 Z M118.249172,2.84217094e-14 C118.249172,32.6440764 91.7686624,59.124586 59.124586,59.124586 C26.4805096,59.124586 0,32.6440764 0,2.84217094e-14 L118.249172,2.84217094e-14 Z M137.750828,194.005605 C137.750828,161.328917 164.198726,134.881019 196.875414,134.881019 C229.552102,134.881019 256,161.361529 256,194.005605 L137.750828,194.005605 Z M137.750828,142.740382 L137.750828,0 L256,0 L137.750828,142.740382 Z" fill="#03363D"></path>
    </g>
    </svg>
  )
);

ZendeskIcon.displayName = "ZendeskIcon";

export const ZendeskIconMetadata = {
  id: "zendesk-icon",
  baseId: "zendesk-icon",
  variant: "default",
  name: "Zendesk Icon",
  category: "aliimam",
  tags: [],
  viewBox: "0 0 256 195",
} as const;

export default ZendeskIcon;
