/**
 * Auto-generated logo component: Replit Icon (default)
 * Do not edit manually
 */

'use client';
import React from 'react';

export interface ReplitIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string;
}

export const ReplitIcon = React.forwardRef<SVGSVGElement, ReplitIconProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 256 308"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g>
        <path d="M0,19.2 C0,8.59648 8.59648,-2.13162821e-14 19.2,-2.13162821e-14 L108.8,-2.13162821e-14 C119.40352,-2.13162821e-14 128,8.59648 128,19.2 L128,102.4 L19.2,102.4 C8.59648,102.4 0,93.80352 0,83.2 L0,19.2 Z M128,102.4 L236.8,102.4 C247.40352,102.4 256,110.99648 256,121.6 L256,185.6 C256,196.20352 247.40352,204.8 236.8,204.8 L128,204.8 L128,102.4 Z M0,224 C0,213.39648 8.59648,204.8 19.2,204.8 L128,204.8 L128,288 C128,298.60352 119.40352,307.2 108.8,307.2 L19.2,307.2 C8.59648,307.2 0,298.60352 0,288 L0,224 Z" fill="#F26207"></path>
    </g>
    </svg>
  )
);

ReplitIcon.displayName = "ReplitIcon";

export const ReplitIconMetadata = {
  id: "replit-icon",
  baseId: "replit-icon",
  variant: "default",
  name: "Replit Icon",
  category: "aliimam",
  tags: [],
  viewBox: "0 0 256 308",
} as const;

export default ReplitIcon;
