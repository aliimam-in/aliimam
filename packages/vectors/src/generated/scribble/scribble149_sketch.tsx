
'use client';
import React from 'react';

export interface Scribble149SketchProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string; 
}

export const Scribble149Sketch = React.forwardRef<SVGSVGElement, Scribble149SketchProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 96 96"
      fill="currentColor"
      className={className}
      xmlns="http://www.w3.org/2000/svg" 
      {...props}
    >
      <defs></defs><title>scribble final version</title><g id="Layer_1" data-name="Layer 1"><path  d="M64.17,34.62c-9.56,1.55-15.29-9.3-15.29-9.3l2.68-3.52A16,16,0,0,0,58,29.62c4.61,2.62,6.78,1.86,6.78,1.86Z"/><path  d="M26,69.17c.17-.79,1.56-1.76,2.18-2.14a6.39,6.39,0,0,1,6.87.81c6.17,5.52-6.21,6.87-3.42,12,.69,1.28-5.05,6.4-5.05,6.4S24.31,75.6,25.28,71.79,26,69.16,26,69.17Z"/><path  d="M65.89,24.36a36.23,36.23,0,0,0,1.59-4.47,15,15,0,0,0,.59-4.22c0-.33-.07-.58-.08-.87a5.14,5.14,0,0,0-.21-.69,1.77,1.77,0,0,0-.28-.51,2.39,2.39,0,0,0-.45-.5,2.91,2.91,0,0,0-2.65-.6c-1.71.34-3.61,1.71-5.38,3.14a55,55,0,0,0-8.67,9.3,55.77,55.77,0,0,0-6.21,11.23,91,91,0,0,0-3.82,11c-.7,2.38-1.2,4.82-1.85,7.23l-2,7.19c-.39,1.47-1,1.24-1.52-.2a16.57,16.57,0,0,1-.75-7c1.7-10.29,4.7-20.54,10.77-29.66C46.88,22,48.82,19.47,51,17A37.71,37.71,0,0,1,58.34,10,14.14,14.14,0,0,1,64,7.67,8.15,8.15,0,0,1,67.58,8a7.65,7.65,0,0,1,3,1.83,8.7,8.7,0,0,1,1.16,1.44,8,8,0,0,1,.73,1.62l.24.82.12.8a10.6,10.6,0,0,1,.1,1.57,21.09,21.09,0,0,1-.89,5.67,51.77,51.77,0,0,1-5.46,12.11c-3.42,5.71-7.66,10.77-11.8,15.75-3.25,3.86-6.41,7.66-9.81,11.31-.74.8-1.45,1.61-2.24,2.39A21.24,21.24,0,0,1,39.32,66a5,5,0,0,1-2.06.64,4.15,4.15,0,0,1-.75,0c-.31,0-.24-.43.1-1.14a14.64,14.64,0,0,1,1.83-3c.7-.87,1.31-1.81,2.05-2.64l2.16-2.57L47,52.16c2.89-3.52,5.77-7.1,8.47-10.79,2-2.72,3.88-5.55,5.68-8.36a52.63,52.63,0,0,0,4.63-8.69Z"/><path  d="M22.63,80c0-.15,0-.36-.11-.45l0-.27-.07-.54-.11-1.1c-.06-.73-.11-1.47-.13-2.21a27,27,0,0,1,.25-4.62,12.9,12.9,0,0,1,.84-3.61,9.85,9.85,0,0,1,2.17-3A8.16,8.16,0,0,1,27,63a13.54,13.54,0,0,1,1.56-.7,9.05,9.05,0,0,1,6.75,0,10,10,0,0,1,4.53,3.65,7.43,7.43,0,0,1,1.29,3.34c0,.29,0,.59,0,.88a8.24,8.24,0,0,1-.11.85,6.68,6.68,0,0,1-.44,1.62c-.25.65-.62.47-1.17-.09-.28-.28-.58-.65-.95-1a13.47,13.47,0,0,1-1.05-1.14,12.94,12.94,0,0,0-1.25-1.63,5.6,5.6,0,0,0-1.41-.89,8,8,0,0,0-1.51-.33,1.7,1.7,0,0,1-.39,0,1.85,1.85,0,0,0-.42-.11,4.39,4.39,0,0,0-.93-.11,1.78,1.78,0,0,0-.6.06,1.81,1.81,0,0,0-.6.18c-.17.09-.45.19-.54.26a1.79,1.79,0,0,0-.35.28,6.58,6.58,0,0,0-1.17,2.09,19.89,19.89,0,0,0-1.07,7.3l0,1.22,0,.75c0,.43.07.88.14,1.34l.15.92.09.49v.06l0,0,.07-.07.13-.15,1.64-1.78.74-.84,1.05-1.13A17.26,17.26,0,0,1,33.72,76,8.41,8.41,0,0,1,35,75.2a3.3,3.3,0,0,1,2.39-.32,1.31,1.31,0,0,1,.92,1.27,2.72,2.72,0,0,1,0,.81,3.22,3.22,0,0,1-.59,1,2.24,2.24,0,0,1-.6.68,23.86,23.86,0,0,0-3.27,2.68l-.9.86-.23.22c-.08.07-.12.13-.34.32l-1,1c-.63.67-1.26,1.3-1.88,2L27.67,88l-2.2,2.58c-.41-1.71-.83-3.43-1.25-5.14A16.73,16.73,0,0,1,22.51,80Z"/><path  d="M58,31.4a19.93,19.93,0,0,1-3-2.17c-.32-.31-.7-.53-1-.85a15.82,15.82,0,0,1-2.5-3.89c-.14-.31-.26-.62-.41-.92a3.88,3.88,0,0,1-.4-.91,11.37,11.37,0,0,1-.33-1.24,4.31,4.31,0,0,1,.06-1.29c.06-.27.55-.28,1.12-.16a2.65,2.65,0,0,1,1.47.65,13,13,0,0,1,1.78,3.53A5.16,5.16,0,0,0,56.51,26a14.44,14.44,0,0,0,2.33,1.66,10.86,10.86,0,0,0,1.62.76A9.45,9.45,0,0,0,63.1,29a5.36,5.36,0,0,0,2-.4,1,1,0,0,1,.42-.06c.51,0,.74.25,1.07,1,.07.17.16.34.22.51a1.39,1.39,0,0,1-.36,1.58,3.93,3.93,0,0,1-2.39,1.15,7.57,7.57,0,0,1-2.6,0,14.48,14.48,0,0,1-3.53-1.35Z"/></g>
    </svg>
  )
);

Scribble149Sketch.displayName = "Scribble149Sketch";

export const Scribble149SketchMetadata = {
  id: "scribble149_sketch",
  baseId: "scribble149",
  variant: "sketch",
  name: "Scribble149",
  category: "scribble",
  tags: [],
  viewBox: "0 0 96 96",
} as const;

export default Scribble149Sketch;
