
'use client';
import React from 'react';

export interface Scribble29SketchProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string; 
}

export const Scribble29Sketch = React.forwardRef<SVGSVGElement, Scribble29SketchProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 96 96"
      fill="currentColor"
      className={className}
      xmlns="http://www.w3.org/2000/svg" 
      {...props}
    >
      <defs></defs><title>scribble final version</title><g id="Layer_1" data-name="Layer 1"><path  d="M66.28,80.38c.2.4-.26.72-1,1.09a1.69,1.69,0,0,1-1.08,0,6.42,6.42,0,0,1-.62-.78,5.36,5.36,0,0,1-.37-1.64c0-.42-.08-.87-.1-1.27s0-.77,0-1A8.8,8.8,0,0,1,63.64,74a19.41,19.41,0,0,1,2.82-5,4,4,0,0,1,1-.87l3.62-2.53a1.43,1.43,0,0,0,.56-.51,6.44,6.44,0,0,1,2.16-1.21c.54-.19,1.1-.36,1.55-.49a5.81,5.81,0,0,1,1-.12,18.4,18.4,0,0,1,2,.17,14.93,14.93,0,0,1,1.91.33c.17.05.37,0,.51.22.34.54.71.48,1.08.55a5.06,5.06,0,0,1,1.44.62,2.37,2.37,0,0,1,.56.75c.16.25-.16.95-.24,1-.27.19-.19.57-.26.88a2.49,2.49,0,0,1-.08.32,6.71,6.71,0,0,1-1.39.6,14.42,14.42,0,0,1-1.4.51h-.53a10.84,10.84,0,0,0-1.11.14l-.6.1-.53.16-.64.2a1.13,1.13,0,0,1-.55,0,5.78,5.78,0,0,0-1.17.54,12,12,0,0,0-1.09.68,2.91,2.91,0,0,1-1.43.51.33.33,0,0,0-.27.14l-1.07.78-.56.38-.52.45a3.25,3.25,0,0,0-.48.5l-.4.57a2.09,2.09,0,0,1-.22.16.89.89,0,0,0-.26.21,9,9,0,0,0-1.4,3,4.87,4.87,0,0,1-.42.81,4.18,4.18,0,0,0-.29.9c0,.09-.16.25-.33.43S66.25,80.3,66.28,80.38Z"/><path  d="M44.8,74.45a2.86,2.86,0,0,1-.87,2.47c-.21.25-.91.32-1,0-.27-.52-.56-1-.8-1.61a5.45,5.45,0,0,1-.25-1.09A19.23,19.23,0,0,1,41.48,72a22.52,22.52,0,0,1-.1-2.58V68.29l.06-.9a21.22,21.22,0,0,1,1.11-5.62A37.35,37.35,0,0,1,48,51.45a10.94,10.94,0,0,1,1.76-1.9c2.32-1.87,4.53-3.75,6.88-5.53a4.68,4.68,0,0,0,1.11-1A7.06,7.06,0,0,1,59.09,42a28.27,28.27,0,0,1,2.73-1.55,30,30,0,0,1,2.95-1.28,16,16,0,0,1,1.84-.55,33.65,33.65,0,0,1,8-.42,2.11,2.11,0,0,1,1.14.27,4.5,4.5,0,0,0,2.38.77,11.7,11.7,0,0,1,1.63.48c.52.23,1,.51,1.53.78a6.92,6.92,0,0,1,1.26,1.22c.35.37-.16.91-.33.93-.51.06-.29.51-.37.79a.92.92,0,0,1-.11.29,29.2,29.2,0,0,0-2.9.35c-1.41.28-2.74.61-3,.65a37,37,0,0,0-3.76.57,22.72,22.72,0,0,0-4.18,1.42,5.37,5.37,0,0,1-1.3.3,19.34,19.34,0,0,0-2.82,1.53c-1.33.81-2.57,1.77-2.67,1.83a18.25,18.25,0,0,0-1.54,1.06,6.81,6.81,0,0,1-1.7.87,1.71,1.71,0,0,0-.62.44c-.79.75-1.59,1.47-2.39,2.21a31.53,31.53,0,0,0-2.38,2.25,10.19,10.19,0,0,0-1.06,1.3c-.33.44-.65.9-1,1.37a5.89,5.89,0,0,1-.42.45,2.53,2.53,0,0,0-.49.6,23.31,23.31,0,0,0-3.1,7.3,21.57,21.57,0,0,0-1,4.17A10.63,10.63,0,0,0,44.8,74.45Z"/><path  d="M14,66.9a5.19,5.19,0,0,1,0,1.12,5.07,5.07,0,0,1-.2,1,7.29,7.29,0,0,1-1,1.92c-.28.39-1.16.4-1.32-.07-.29-.9-.59-1.83-.83-2.79a14.37,14.37,0,0,1-.13-1.85c0-.5,0-1.09-.08-1.72s0-1.32.05-2a51.59,51.59,0,0,1,.82-7.59,34.88,34.88,0,0,1,2.88-9A64.39,64.39,0,0,1,25,30.32a28,28,0,0,1,3.12-2.84c3.83-2.93,7.63-5.8,11.64-8.47a8,8,0,0,0,1.95-1.43A12.92,12.92,0,0,1,44,16c.64-.34,1.33-.78,2.12-1.15l2.47-1.12a41.86,41.86,0,0,1,8-2.52,54.76,54.76,0,0,1,13-.46,3.89,3.89,0,0,1,1.82.34,9.8,9.8,0,0,0,3.84,1.2c.89.24,1.77.47,2.64.75s1.7.69,2.53,1.06a13.15,13.15,0,0,1,2.16,1.7c.62.53-.15,1.2-.43,1.21-.82,0-.41.62-.5,1a.87.87,0,0,1-.15.38,7.5,7.5,0,0,1-1.54-.13l-.71-.1-.82,0-1.76-.07-1.79.09a13,13,0,0,0-1.61.14,18.26,18.26,0,0,1-1.93.2c-.06,0-.9.1-2.14.1a37.47,37.47,0,0,0-4.59.53,39.83,39.83,0,0,0-7.5,2.05,12.94,12.94,0,0,1-2.27.53c-.09,0-.76.31-1.71.72s-2.16,1.06-3.38,1.66c-2.43,1.25-4.74,2.71-4.93,2.8-1,.5-1.87,1.12-2.82,1.66A14,14,0,0,1,39,30a4.54,4.54,0,0,0-1.11.73,102.9,102.9,0,0,0-8.71,7.35c-.67.7-1.32,1.43-2,2.16l-1.8,2.28c-.23.27-.49.51-.73.77a11.76,11.76,0,0,0-.86,1q-2,2.85-3.64,5.85c-.58,1-1.08,2-1.61,3.05s-.89,2.12-1.25,3.2a11.55,11.55,0,0,1-.52,1.32c-.25.6-.58,1.31-.81,2.05s-.52,1.48-.71,2.12S15,63.09,15,63.4a16.79,16.79,0,0,1-.6,1.76A6.57,6.57,0,0,0,14,66.9Z"/></g>
    </svg>
  )
);

Scribble29Sketch.displayName = "Scribble29Sketch";

export const Scribble29SketchMetadata = {
  id: "scribble29_sketch",
  baseId: "scribble29",
  variant: "sketch",
  name: "Scribble29",
  category: "scribble",
  tags: [],
  viewBox: "0 0 96 96",
} as const;

export default Scribble29Sketch;
