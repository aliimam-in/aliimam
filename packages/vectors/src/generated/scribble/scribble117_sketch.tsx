
'use client';
import React from 'react';

export interface Scribble117SketchProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string; 
}

export const Scribble117Sketch = React.forwardRef<SVGSVGElement, Scribble117SketchProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 96 96"
      fill="currentColor"
      className={className}
      xmlns="http://www.w3.org/2000/svg" 
      {...props}
    >
      <defs></defs><title>scribble final version</title><g id="Layer_1" data-name="Layer 1"><path  d="M43.41,23.44A13.21,13.21,0,0,0,39,29.63a9.12,9.12,0,0,0-.27,2.65,9.56,9.56,0,0,0,1.15,4.65,10.87,10.87,0,0,0,3.36,4,12.65,12.65,0,0,0,4.47,2c.51.12,1,.27,1.55.43s1.06.27,1.61.37a11.36,11.36,0,0,0,1.65.14,13.45,13.45,0,0,0,1.68,0c.7,0,.62,0,0,.26a14.86,14.86,0,0,1-3,1,16.09,16.09,0,0,1-3.77.67A17.14,17.14,0,0,1,43.69,45a13.86,13.86,0,0,1-6.46-4.57,15.18,15.18,0,0,1-2.39-4.37,12.72,12.72,0,0,1-.77-5,16.17,16.17,0,0,1,4.29-9.59l1.09-1.14c.38-.36.77-.69,1.15-1s.8-.65,1.19-1,.83-.63,1.29-.94a16.88,16.88,0,0,1,9.21-2.48,23.79,23.79,0,0,1,7.25.84,12.82,12.82,0,0,1,1.55.52c1.85.9,2.15,1.7,1.8,2.89a6.94,6.94,0,0,1-.25.81c-.41.84-1.43,1-2.62.8a35.34,35.34,0,0,0-5.16-.9,17.89,17.89,0,0,0-5,.25A13.05,13.05,0,0,0,46.6,21.3a5,5,0,0,0-.75.42l-.36.22-.41.31-.82.62-.76.66Z"/><path  d="M65.71,44.22a38.37,38.37,0,0,0,.52-5.72,30.42,30.42,0,0,0-.39-5.59,17.67,17.67,0,0,0-4.31-9.42,15.26,15.26,0,0,0-6.44-3.77,12.36,12.36,0,0,0-3.21-.7,12.65,12.65,0,0,0-3.24.17,15.43,15.43,0,0,0-6.18,2.71,16.09,16.09,0,0,0-4.57,5.18,12.36,12.36,0,0,0-1.68,6.5,9.53,9.53,0,0,0,2,5.47,11.69,11.69,0,0,0,4.9,3.52,18,18,0,0,0,8,1.09c.68-.08,1.35-.18,2-.29s1.35-.28,2-.5a19.89,19.89,0,0,0,2-.7,6.53,6.53,0,0,1,1-.33,7.85,7.85,0,0,0,1-.38c1.72-.63,2-.12,1.2,1.71a9.69,9.69,0,0,1-.81,1.61,3.43,3.43,0,0,0-.28.47l-.36.43a6.54,6.54,0,0,1-.82.85,11.25,11.25,0,0,1-5,2.63,21.09,21.09,0,0,1-11.27-.32,18,18,0,0,1-9.57-7.21A16.94,16.94,0,0,1,30,35.76a16.55,16.55,0,0,1,.09-6.12,19.28,19.28,0,0,1,5.53-10,24.07,24.07,0,0,1,11.68-6.54,18.86,18.86,0,0,1,7-.2,28.59,28.59,0,0,1,3.28.81,11.35,11.35,0,0,1,1.57.57c.52.22,1,.4,1.55.66a22,22,0,0,1,6,4.43,23.25,23.25,0,0,1,4,6.15,30,30,0,0,1,2,6.79,41.85,41.85,0,0,1,.56,6.86,50.35,50.35,0,0,1-.77,8.26,41,41,0,0,1-.83,4,35.17,35.17,0,0,1-1.12,4,40.89,40.89,0,0,1-5.66,11.23c-.31.43-.58.88-.92,1.29l-1,1.21a22.6,22.6,0,0,1-2.12,2.35c-.77.73-1.52,1.48-2.3,2.18L56,75.65A21.17,21.17,0,0,1,54,77c-.7.43-1.37.88-2.08,1.29l-2.18,1.08a15.61,15.61,0,0,1-2.21,1l-2.29.85c-.38.14-.75.31-1.14.42L42.9,82a32.14,32.14,0,0,1-4.75,1,27.68,27.68,0,0,1-4.25.5c-1.33,0-2.44,0-3.36,0a8.5,8.5,0,0,1-2.22-.67,4.07,4.07,0,0,1-2.21-2.12,6.35,6.35,0,0,1-.54-1.21c-.29-1.27,1.89-2.25,5.32-2.64A46.07,46.07,0,0,0,45.53,73.7,29.48,29.48,0,0,0,57.09,65a33.23,33.23,0,0,0,5.5-9.62,48.12,48.12,0,0,0,2.94-11.16Z"/></g>
    </svg>
  )
);

Scribble117Sketch.displayName = "Scribble117Sketch";

export const Scribble117SketchMetadata = {
  id: "scribble117_sketch",
  baseId: "scribble117",
  variant: "sketch",
  name: "Scribble117",
  category: "scribble",
  tags: [],
  viewBox: "0 0 96 96",
} as const;

export default Scribble117Sketch;
