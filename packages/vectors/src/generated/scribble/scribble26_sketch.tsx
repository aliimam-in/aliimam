
'use client';
import React from 'react';

export interface Scribble26SketchProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string; 
}

export const Scribble26Sketch = React.forwardRef<SVGSVGElement, Scribble26SketchProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 96 96"
      fill="currentColor"
      className={className}
      xmlns="http://www.w3.org/2000/svg" 
      {...props}
    >
      <defs></defs><title>scribble final version</title><g id="Layer_1" data-name="Layer 1"><path  d="M59.68,81.49a42.71,42.71,0,0,1-10.32,2.36,43.32,43.32,0,0,1-10.49-.18,36,36,0,0,1-10.16-2.93A26.39,26.39,0,0,1,20,74.32a22.47,22.47,0,0,1-5.65-14.59A29.22,29.22,0,0,1,15,53,27.9,27.9,0,0,1,17,46.61,31.71,31.71,0,0,1,24.88,35.9a21.82,21.82,0,0,1,5.76-3.62,14.78,14.78,0,0,1,7-1.13,17.28,17.28,0,0,1,12,6.14,17.5,17.5,0,0,1,2.92,5.27,20.12,20.12,0,0,1,1.29,5.81,13.33,13.33,0,0,1-.16,3.08,10.88,10.88,0,0,1-1,3A11.17,11.17,0,0,1,48.38,59a12.68,12.68,0,0,1-7.69,1.74,10.52,10.52,0,0,1-3.95-1.24,7.9,7.9,0,0,1-3.12-3.06,7.55,7.55,0,0,1-.86-4.3,9.76,9.76,0,0,1,1.24-4,8,8,0,0,1,1.22-1.67,9.09,9.09,0,0,1,1.65-1.3,6.4,6.4,0,0,1,4.07-.92,3.8,3.8,0,0,1,1.74.62c.32.23.42.38.35.47s-.94,0-2.09.31a4.58,4.58,0,0,0-3,2.89A11,11,0,0,0,37.16,51a6,6,0,0,0,.08,2.47,3.08,3.08,0,0,0,.91,1.57,5.19,5.19,0,0,0,2,1.11,7.78,7.78,0,0,0,5.14-.35A6.9,6.9,0,0,0,49,52.43a8.78,8.78,0,0,0,.29-5.35,19.83,19.83,0,0,0-.91-2.94c-.21-.49-.42-1-.65-1.46s-.44-.83-.7-1.23a12.25,12.25,0,0,0-4.23-3.84,14.78,14.78,0,0,0-5.58-1.78,10.73,10.73,0,0,0-2.89.09,11.45,11.45,0,0,0-2.81.87,18,18,0,0,0-3.25,2,21.7,21.7,0,0,0-2.88,2.72,23.24,23.24,0,0,0-4.26,6.9A22.43,22.43,0,0,0,20,52.26c-.15.64-.24,1.33-.34,2a17.61,17.61,0,0,0-.2,2,21.53,21.53,0,0,0,.77,7.88,18.21,18.21,0,0,0,1.7,3.89,19,19,0,0,0,2.63,3.32,21.7,21.7,0,0,0,7.33,4.64,34.66,34.66,0,0,0,17.87,1.64A34.23,34.23,0,0,0,55.2,76.4a29.46,29.46,0,0,0,5.12-2.14,29.07,29.07,0,0,0,4.66-3,29.84,29.84,0,0,0,4-3.83,32.33,32.33,0,0,0,7.42-15.07,31.63,31.63,0,0,0,.47-8.46A29,29,0,0,0,75,35.69,27.39,27.39,0,0,0,71.86,30a32,32,0,0,0-4.3-4.76,29,29,0,0,0-5.15-3.59,29,29,0,0,0-5.78-2.23,29.27,29.27,0,0,0-5.37-.72c-3.39-.19-5.64.11-7.28,0a6.87,6.87,0,0,1-2.1-.29,5.14,5.14,0,0,1-1.61-.75,8.73,8.73,0,0,1-1.38-.93c-.57-.59-.19-1.37,1.08-2.26a15.81,15.81,0,0,1,6.51-2.25A33,33,0,0,1,53.39,12a31.77,31.77,0,0,1,7,1.35,30.22,30.22,0,0,1,6.64,3,33.24,33.24,0,0,1,3,2.06c.49.37.94.78,1.41,1.18a17.9,17.9,0,0,1,1.37,1.23l1.29,1.3c.44.44.81.92,1.21,1.38a34.31,34.31,0,0,1,2.24,2.93A35.62,35.62,0,0,1,81.11,33a36,36,0,0,1,2.56,14.61A38.57,38.57,0,0,1,76.8,68.21a36.41,36.41,0,0,1-17,13.42Z"/></g>
    </svg>
  )
);

Scribble26Sketch.displayName = "Scribble26Sketch";

export const Scribble26SketchMetadata = {
  id: "scribble26_sketch",
  baseId: "scribble26",
  variant: "sketch",
  name: "Scribble26",
  category: "scribble",
  tags: [],
  viewBox: "0 0 96 96",
} as const;

export default Scribble26Sketch;
