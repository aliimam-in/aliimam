
'use client';
import React from 'react';

export interface Scribble86SketchProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  className?: string; 
}

export const Scribble86Sketch = React.forwardRef<SVGSVGElement, Scribble86SketchProps>(
  ({ size = 24, className = '', ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 96 96"
      fill="currentColor"
      className={className}
      xmlns="http://www.w3.org/2000/svg" 
      {...props}
    >
      <defs></defs><title>scribble final version</title><g id="Layer_1" data-name="Layer 1"><path  d="M15.44,78.13l.82-.1a2.47,2.47,0,0,0,.78-.17l1.4-.56a19.52,19.52,0,0,0,2.71-1.23c1.73-.93,3.43-1.89,5.12-2.88s3.37-2,5-3c.83-.52,1.66-1,2.48-1.58l2.43-1.69a95.78,95.78,0,0,0,9.4-7.49A33.84,33.84,0,0,0,50,54.58,13.21,13.21,0,0,0,51.86,51a10,10,0,0,0,.29-1.35,9.64,9.64,0,0,0,.06-1.42,6,6,0,0,0-.28-1.39c-.15-.45-.37-.87-.53-1.32a5.64,5.64,0,0,1-.64-.85L50.32,44c-.3-.44-.62-.77-.93-1.16s-.62-.75-.93-1.11l-1-1c-.64-.7-1.3-1.32-2-2s-1.33-1.25-2-1.84c-2.74-2.37-5.55-4.55-8.41-6.65Q26.49,24,17.39,18.66c-.89-.53-2.71,2.37-1.9,2.94,5.62,4,11.07,8.33,16.25,12.87,2.58,2.27,5.09,4.6,7.43,7,1.17,1.21,2.31,2.43,3.34,3.68l.77.93.68.94a10,10,0,0,1,.65.94c.14.28.47.71.36.7.09,0,0,0,0,0s0,0,0,0a6.56,6.56,0,0,1-.94,1.4,26.87,26.87,0,0,1-3.59,3.4,85.71,85.71,0,0,1-9,6.2c-3.19,1.9-6.35,3.9-9.63,5.71s-6.77,3.3-9.49,5.74C11.91,71.72,14.41,78.19,15.44,78.13Z"/><path  d="M43.75,84.26c4.7-1.19,8.26-3.84,12.12-6.14s7.55-4.8,11.15-7.5A125.26,125.26,0,0,0,77.54,62a50.59,50.59,0,0,0,4.9-5.18,23.66,23.66,0,0,0,2.23-3.22,11.25,11.25,0,0,0,1.54-4.62,6.85,6.85,0,0,0-.53-3.31c-.12-.27-.27-.53-.41-.78a1.49,1.49,0,0,0-.18-.27L85,44.38l-.56-.77A31.7,31.7,0,0,0,82,40.79c-.8-.88-1.64-1.66-2.47-2.49L77,36C73.55,33,70,30.39,66.3,27.84a257.17,257.17,0,0,0-22.6-14c-1.08-.6-2.8,2.36-1.79,3A253.24,253.24,0,0,1,63,32.2c3.36,2.73,6.62,5.57,9.65,8.54L74.89,43c.68.77,1.4,1.53,2,2.3a26.48,26.48,0,0,1,1.73,2.34l.39.58.09.15,0,.07c.15-.09-.32.22.35-.2h0l.09-.05-.06,0a1.06,1.06,0,0,1-.23,0,1.11,1.11,0,0,1-.27,0,.12.12,0,0,0,0,0,5.14,5.14,0,0,1-.73,1.49,16.92,16.92,0,0,1-1.65,2.1A44.84,44.84,0,0,1,72.28,56,119.07,119.07,0,0,1,62,63.52c-3.58,2.35-7.17,4.75-10.86,7s-7.61,4.14-10.76,6.9C39.84,78.05,42.58,84.42,43.75,84.26Z"/></g>
    </svg>
  )
);

Scribble86Sketch.displayName = "Scribble86Sketch";

export const Scribble86SketchMetadata = {
  id: "scribble86_sketch",
  baseId: "scribble86",
  variant: "sketch",
  name: "Scribble86",
  category: "scribble",
  tags: [],
  viewBox: "0 0 96 96",
} as const;

export default Scribble86Sketch;
