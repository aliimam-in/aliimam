...
const transformers = {
  ...
  readyForJSX(svgRaw: string) {
    try {
      ...
      // Remove script tags or any potentially dangerous content
      $("script").remove();
      ...
      // Validate the output is valid JSX
      if (result.includes("catch") || !result.startsWith("<svg")) {
        throw new Error("Invalid JSX output from SVG transformation");
      }

      return result;
    } catch (error) {
      console.error(
        `Error in readyForJSX: error\nInput SVG: ${svgRaw}`
      );
      throw error;
    }
  },
    ...
    if (iconSetNode.type === "FRAME" || iconSetNode.type === "GROUP") {
      ...
      iconSetNode.children.forEach((iconGroupNode) => {
        if (iconGroupNode.type === "COMPONENT") {
          ...
          const combinedCategory = subCategory === "ali" ? topLevelCategory : `${topLevelCategory}/${subCategory}`;
...
export async function generateReactComponents(icons: IIcons) {
  ...
  for (const category in iconsByCategory) {
    ...
    const categoryIndexPath = path.resolve(
      currentTempDir,
      "src",
      category,
      "index.ts"
